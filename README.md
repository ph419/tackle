...
| `tackle-harness loop <plan> [--executor=local\|claude\|glm] [--loop-id=X] [--max-iters=N]` | 以 Node 进程驱动 Agentic Loop（v0.3.4+，provider 可替换） |
...
v0.3.4 起 Agentic Loop 的循环载体可从「Claude 会话内伪代码」升级为 **Node 进程级 driver**（`tackle loop`），把 Claude/任意 provider 退化为可替换的无状态 executor：

```bash
# 用 mock executor 冒烟（不调真模型，免费验证收敛）
tackle-harness loop test/fixtures/todo-cli-smoke.md --executor=local

# 真实驱动 Claude Code 逐轮写代码
tackle-harness loop docs/plan/my-plan.md --executor=claude

# 智谱 GLM Coding Plan（享套餐额度，走 claude CLI 中转，合规）
...
- **provider 可替换**：`--executor=local|claude|glm` 切换时 driver/engine 零改动，解耦点是统一的 `executor.run()` 契约（新增 executor 仅在 `loop-executor` 注册一行）
